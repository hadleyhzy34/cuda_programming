...
// 1. Preprocesses points, filtering any that are too close based on a dynamic
// threshold
__global__ void preprocess_points(Point *points, int n,
                                  float filter_threshold) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < n) {
    float dist_sq =
        points[idx].x * points[idx].x + points[idx].y * points[idx].y;
...
__global__ void detect_hazard(const Point *points, int n,
                              int *out_hazard_flag) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  // Simple logic: if a valid point is found very close, flag it as a hazard.
  // This is a reduction, but we simplify it by having each thread potentially
  // write.
...
public:
  ...
  ~SensorProcessor();
  void process_frame(const std::vector<Point> &frame_data,
                     float new_filter_threshold);
...
private:
  ...
  cudaGraphNode_t
      preprocess_node_; // We need a handle to this node to update it
  cudaStream_t stream_;
};

SensorProcessor::SensorProcessor(int points_per_frame)
    : points_per_frame_(points_per_frame) {
...
void SensorProcessor::build_graph() {
  ...
  // or more commonly, the H2D copy happens outside the graph launch if data
  // changes every frame. For simplicity in this example, we'll do the memcpy
  // outside the main graph launch.
  ...
  void *preprocess_args[] = {(void *)&d_points_, (void *)&points_per_frame_,
                             (void *)&initial_threshold};
  preprocess_params.kernelParams = preprocess_args;
  CHECK_CUDA(cudaGraphAddKernelNode(&preprocess_node_, main_graph_, nullptr, 0,
                                    &preprocess_params));
  ...
  void *detect_args[] = {(void *)&d_points_, (void *)&points_per_frame_,
                         (void *)&d_hazard_flag_};
  detect_params.kernelParams = detect_args;
  CHECK_CUDA(cudaGraphAddKernelNode(&hazard_detect_node, main_graph_,
                                    &preprocess_node_, 1, &detect_params));
  ...
  log_params.gridDim = dim3(1);
  log_params.blockDim = dim3(1);
  void *log_args[] = {(void *)&d_hazard_log_counter_};
  log_params.kernelParams = log_args;
  CHECK_CUDA(cudaGraphAddKernelNode(&log_node, graph_if_hazard, nullptr, 0,
                                    &log_params));
  ...
  CHECK_CUDA(cudaGraphConditionalHandleCreate(&cond_handle, d_hazard_flag_, 1,
                                              cudaGraphConditionalDefault));
  ...
  CHECK_CUDA(cudaGraphAddNode(&conditional_node, main_graph_,
                              &hazard_detect_node, 1, &conditional_params));
...
void SensorProcessor::process_frame(const std::vector<Point> &frame_data,
                                    float new_filter_threshold) {
<<<<<<< HEAD
  CHECK_CUDA(cudaMemsetAsync(d_hazard_flag_, 0, sizeof(int), stream_));
  CHECK_CUDA(cudaMemcpyAsync(d_points_, frame_data.data(),
                             frame_data.size() * sizeof(Point),
...
>>>>>>> a5eb08724efa9d3eab0402ac9dae2434dda22bc6
  ...
  preprocess_params.blockDim = block_;
  void *preprocess_args[] = {(void *)&d_points_, (void *)&points_per_frame_,
                             (void *)&new_filter_threshold};
  preprocess_params.kernelParams = preprocess_args;
  CHECK_CUDA(cudaGraphExecKernelNodeSetParams(instance_, preprocess_node_,
                                              &preprocess_params));
...
<<<<<<< HEAD
  ...
  // Copy hazard flag back to host
  int hazard_flag = 0;
  CHECK_CUDA(cudaMemcpyAsync(&hazard_flag, d_hazard_flag_, sizeof(int),
                             cudaMemcpyDeviceToHost, stream_));
  CHECK_CUDA(cudaStreamSynchronize(stream_));

  // If hazard detected, launch logging kernel
  if (hazard_flag) {
...
=======
  // --- GRAPH LAUNCH ---
  // Launch the entire updated pipeline. The GPU will handle the conditional
  // logic internally.
  CHECK_CUDA(cudaGraphLaunch(instance_, stream_));
...
SensorProcessor::~SensorProcessor() {
  ...
  // The conditional subgraphs are owned by the main graph, no need to destroy
  // them separately
...
int main() {
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    if (i == 3 || i == 7) {
      ...
      frame_data[100].x = 1.0f;
      frame_data[100].y = 1.0f;
    }

    // Simulate adapting to the environment by changing the filter
    float filter_threshold = 5.0f - 0.2f * i;
    std::cout << "  Updating filter threshold to: " << filter_threshold
              << std::endl;
