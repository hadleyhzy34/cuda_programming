...
int main() {
    ...
    for(int i = 0; i < TOTAL_ELEMENTS; ++i) h_in_pinned[i] = static_cast<float>(i);
    ...
    for (int i = 0; i < NUM_CHUNKS; ++i) {
        ...
        CHECK_CUDA(cudaMemcpy(d_data, h_in_pageable.data() + offset, CHUNK_BYTES, cudaMemcpyHostToDevice));
        ...
        CHECK_CUDA(cudaMemcpy(h_out_pageable.data() + offset, d_data, CHUNK_BYTES, cudaMemcpyDeviceToHost));
    }
    ...
    for (int i = 0; i < NUM_CHUNKS; ++i) {
        ...
        CHECK_CUDA(cudaMemcpyAsync(d_data, h_in_pinned + offset, CHUNK_BYTES, cudaMemcpyHostToDevice, streams[stream_idx]));
        init_data<<<grid, block, 0, streams[stream_idx]>>>(d_data, CHUNK_ELEMENTS, offset);
        process_A<<<grid, block, 0, streams[stream_idx]>>>(d_data, CHUNK_ELEMENTS);
        process_B<<<grid, block, 0, streams[stream_idx]>>>(d_data, PROCESS_FACTOR, CHUNK_ELEMENTS);
        CHECK_CUDA(cudaMemcpyAsync(h_out_pinned + offset, d_data, CHUNK_BYTES, cudaMemcpyDeviceToHost, streams[stream_idx]));
    }
    ...
    for (int i = 0; i < NUM_STREAMS; ++i) CHECK_CUDA(cudaStreamDestroy(streams[i]));
    ...
    CHECK_CUDA(cudaMemcpyAsync(d_data, h_in_pinned, CHUNK_BYTES, cudaMemcpyHostToDevice, capture_stream));
    init_data<<<grid, block, 0, capture_stream>>>(d_data, CHUNK_ELEMENTS, 0);
    process_A<<<grid, block, 0, capture_stream>>>(d_data, CHUNK_ELEMENTS);
    process_B<<<grid, block, 0, capture_stream>>>(d_data, PROCESS_FACTOR, CHUNK_ELEMENTS);
    CHECK_CUDA(cudaMemcpyAsync(h_out_pinned, d_data, CHUNK_BYTES, cudaMemcpyDeviceToHost, capture_stream));
    ...
    for (int i = 0; i < NUM_CHUNKS; ++i) {
        ...
        // using cudaGraphExecUpdate for each chunk. For this benchmark, we repeatedly
        // process the same chunk to focus on the launch overhead comparison.
